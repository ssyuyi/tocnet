...
class TDH(AnchorFreeHead):
    ...
    def forward(
        ...
        for i in range(len(x)):
            tmp = self.forward_single(x, self.scales[i], self.strides[i], i)
            out.append(tmp)
        res = tuple(map(list, zip(*out)))
        return res
        # return self.AFM_TDH(self.forward_single, x, self.scales, self.strides)

    # multi_apply(self.forward_single, x, self.scales, self.strides)
    #
    ...
    def forward_single1(self, x_cls: Tensor, x_reg: Tensor) -> Tuple[Tensor, ...]:
    ...
    def afm(self, x1, x2):
        n,c,h,w = x1.shape
        inter = torch.cosine_similarity(x1,x2,dim=1)
        inter = torch.unsqueeze(inter, 1)
        inter = torch.reshape(inter,[n,1,h,w])
        xout = x1 + x2*inter
        return xout
    ...
    def fenge(self, x, ix):
        ...
        x_reg = 0
        if ix == 0:
            x_cls = (self.afm(cur, F.interpolate(x[1], [h,w])))
            x_reg = cur
        elif ix == 1:
            x_cls = self.afm(cur, F.interpolate(x[2],[h,w]))
            x_reg = self.afm(cur, F.interpolate(x[0],[h,w]))
        elif ix == 2:
            x_cls = cur
            x_reg = (self.afm(cur, F.interpolate(x[0],[h,w])))
        return x_cls, x_reg
    ...
    def forward_single(self, x: Tensor, scale: Scale,
                       stride: int, ix: int) -> Tuple[Tensor, Tensor, Tensor]:
        """Forward features of a single scale level.
        ...
        """
        x_cls, x_reg = self.fenge(x, ix)
        cls_score, bbox_pred, cls_feat, reg_feat = self.forward_single1(x_cls, x_reg)
