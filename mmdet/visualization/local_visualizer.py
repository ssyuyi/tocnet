...
class DetLocalVisualizer(Visualizer):
    ...
    def _draw_img(self, image, gts, color, mode, thick=2):
        for gt in gts:
            x1 = int(gt[0])
            y1 = int(gt[1])
            x2 = int(gt[2])
            ...
            w = x2-x1
            h = y2-y1
            # whr = int(np.sqrt((h ** 2 + w ** 2))/2)
            ra = 0.8
            if mode == 'rec':
                cv2.rectangle(image,[x1,y1],[x2,y2],color,thickness=thick)
            if mode == 'ovl':
                cv2.ellipse(image,(xc,yc), (int(ra*w), int(ra*h)),0, 0, 360, color, thickness=thick)

    def yilou_and_wujian(self, gt, pd):
        gt_boxes = gt.bboxes
        pd_boxes = pd.bboxes
        loujian = torch.ones(len(gt_boxes),dtype=torch.bool)
        wujian = []
        jdbg = []
        lj = []
        zq = []
        for pd_box in pd_boxes:
            ...
            for idx, gt_box in enumerate(gt_boxes):
                tmp_iou = self.bbox_iou(pd_box, gt_box)
                if tmp_iou>max_iou:
                    max_iou = tmp_iou
                    loujian[idx] = False
            if max_iou > 0.5:
                ...
                wujian.append(pd_box)
            else:
                jdbg.append(pd_box)
        gts = []
        for idx, panduan in enumerate(loujian):
            gts.append(gt_boxes[idx,:])
            if panduan:
                lj.append(gt_boxes[idx,:])
        return wujian, lj, jdbg, zq, gt_boxes
    ...
    def bbox_iou(self, box1, box2):
        ...
        inter = 0
        # 基础练习 矩形面积交
        a1 = max(min(box1[0], box1[2]), min(box2[0], box2[2]))
        a2 = min(max(box1[0], box1[2]), max(box2[0], box2[2]))
        b1 = max(min(box1[1], box1[3]), min(box2[1], box2[3]))
        b2 = min(max(box1[1], box1[3]), max(box2[1], box2[3]))
        if a1 < a2 and b1 < b2:
            inter = (a2 - a1) * (b2 - b1)
        return inter / (area1 + area2 - inter)  # iou = inter / (area1 + area2 - inter)


    def extract_gts(self,image,gt_boxes):
        gts = []
        for gt in gt_boxes:
            x1 = int(gt[0])
            y1 = int(gt[1])
    ...
    def add_datasample(
        ...
        # name = rs[-1]
        # names = os.listdir(f'/home/ubuntu/wangmao_workspace/0620/org/{coco_type}')
        ...
        pd = data_sample.pred_instances

        wujian, lj, jdbg, zq, gt_boxes = self.yilou_and_wujian(gt, pd)
        #
        green = (0,255,0)
        red = (255,0,0)
        blue = (0,0,255)
        ...
        puper = (209,159,232)
        yellow = (255,255,0)
        self._draw_img(image, gt_boxes, green, 'rec')
        self._draw_img(image, zq, red, 'rec')
        self._draw_img(image, wujian, blue, 'ovl', thick=4)

        # gts = self.extract_gts(image,gt_boxes)
        # try:
        #     for idx, gt in enumerate(gts):
        #         mmcv.imwrite(gt[...,::-1],out_file.replace('.',f'_{idx}.'))
        # except:
        #     pass
        ...
        real_p ='/'+tmp[0]
        for tp in tmp[1:-1]:
            real_p = os.path.join(real_p,tp)

        ljpt = os.path.join(real_p,'lj')
        if not os.path.exists(ljpt):
            os.makedirs(ljpt)
        jdbgpt = os.path.join(real_p,'jdbg')
        if not os.path.exists(jdbgpt):
            os.makedirs(jdbgpt)
        wjpt = os.path.join(real_p,'wj')
        if not os.path.exists(wjpt):
            os.makedirs(wjpt)
        pfpt = os.path.join(real_p,'perfect')
        if not os.path.exists(pfpt):
            os.makedirs(pfpt)

        if len(lj)!=0:
            ljp = os.path.join(real_p,'lj',tmp[-1])
            mmcv.imwrite(drawn_img[..., ::-1], ljp)
        if len(jdbg)!=0:
            jdbgp = os.path.join(real_p,'jdbg',tmp[-1])
            mmcv.imwrite(drawn_img[..., ::-1], jdbgp)
        if len(wujian)!=0:
            wjp = os.path.join(real_p,'wj',tmp[-1])
            mmcv.imwrite(drawn_img[..., ::-1], wjp)
        if len(lj)==0 and len(jdbg)==0 and len(wujian)==0:
            pfp = os.path.join(real_p, 'perfect', tmp[-1])
            mmcv.imwrite(drawn_img[..., ::-1], pfp)
        #
        # if len(lj)!=0:
